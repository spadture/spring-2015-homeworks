...
#columns = ('name','excellent_rating','very_good_rating','average_rating','poor_rating','terrible_rating','families_rating', 'couple_rating', 'solo_rating', 'business_rating','sleep_quality','location','service','value','cleanliness'))

base_url = "http://www.tripadvisor.com/"
user_agent = "Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_2) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2272.76 Safari/537.36"

"""New Method for downloading html for each hotel page"""
def get_hotel_page(hotel_name, hotel_url,  datadir='data/'):
	url = base_url + hotel_url
	headers = {'User-Agent': user_agent}
	response = requests.get(url, headers=headers)
	html = response.text.encode('utf-8')
	with open(os.path.join(datadir, hotel_name.replace(' ','-').replace("/",'-') + '-hotel-page.html'), "w") as h:
	    h.write(html)
	return html


"""New method for parsing out the ratings data from each hotel"""
def parse_hotel_page(html, hotel_name, data):
	soup = BeautifulSoup(html)
	ratings_box = soup.find('div', {'class' : 'ratings_and_types concepts_and_filters'})
	ratings =[rating.text for rating in ratings_box('span','compositeCount')]
	ratings = [r.replace(',','') for r in ratings]
	
	endorsements =[endorsement.text for endorsement in ratings_box('div', {'class': 'value'})]
	endorsements = [e.replace(',','') for e in ratings]
	summary = [summary['alt'] for summary in ratings_box('img', {'class': re.compile('^sprite-rating_s_fill')})]
	summary = [s[:-11] for s in summary]
	d1 =dict(zip(['name','excellent_rating','very_good_rating','average_rating','poor_rating','terrible_rating','families_rating', 'couple_rating', 'solo_rating', 'business_rating','sleep_quality','location','service','value','cleanliness'],[hotel_name]+ratings+endorsements+summary))
...
def parse_hotellist_page(html,data):
    ...
    for hotel_box in hotel_boxes:
        ...
        hotel_url = hotel_box.find("a")['href']
        hotel_html = get_hotel_page(hotel_name, hotel_url)
        data = parse_hotel_page(hotel_html,hotel_name,data)
        print data
    ...
    for href in hrefs:
    	if href['class'] == "guiArw sprite-pageNext ":
...
def scrape_hotels(city, state, datadir='data/'):
    ...
    while(city_url != None):
        ...
        city_url, data = parse_hotellist_page(html,data)
    return data
